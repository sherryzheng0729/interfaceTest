    ...
    if len(database) == 0:
        ...
        for db in tree.findall("database"):
            ...
            for tb in db.getchildren():
                ...
                # print(table_name)
                interface = {}
                for inter in tb.getchildren():
                    interface_name = inter.get("name")
                    print(interface_name)
                    sql = {}
                    for data in inter.getchildren():
                        sql_id = data.get("id")
                        # print(sql_id)
                        sql[sql_id] = data.text
                    interface[interface_name] = sql
                table[table_name] = interface
            database[db_name] = table


def get_xml_dict(database_name, table_name,interface_name):
    """
    ...
    """
    set_xml()
    database_dict = database.get(database_name).get(table_name).get(interface_name)
    return database_dict


def get_sql(database_name, table_name, interface_name, sql_id):
    """
    ...
    :return:
    """
    db = get_xml_dict(database_name, table_name,interface_name)
    sql = db.get(sql_id)
    return sql
